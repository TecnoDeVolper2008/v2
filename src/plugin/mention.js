...
const mistralSystemPrompt = "You are Qᴜᴇᴇɴ Aʟʏᴀ👸, an intelligent bot. Someone mentioned your owner. Respond with helpful and friendly assistance.";
...
const mistral = async (m, Matrix) => {
    ...
    if (isBotOrOwnerMentioned) {
        ...
        if (text === `@${botNumber}` || text === `@${ownerNumber}`) {
            await Matrix.sendMessage(m.from, { audio: { url: 'https://telegra.ph/file/9a760094f13afad3b6577.jpg' } }, { quoted: m });
            return;
        }
